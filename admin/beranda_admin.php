...
<div class="konten-admin">
    <div class="container-admin">
        <div class="daftar-jurusan">
            ...
            <a href="beranda_admin.php?toggle=jurusan">Lihat Daftar Jurusan</a>
            <a href="beranda_admin.php?toggle=tambah">Tambah Jurusan</a>
        </div>

        <div class="daftar-siswa">
            <h3>Daftar Calon Siswa</h3>
            <a href="beranda_admin.php?toggle=siswa">Lihat Calon Siswa</a>
        </div>
        <div class="daftar-kebutuhan">
            <h3>Daftar Kebutuhan</h3>
            <a href="beranda_admin.php?toggle=kebutuhan">Daftar Kebutuhan Khusus</a>
    ...
    <?php if ($tampilkan_edit_jurusan): ?>
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_GET['action']) && $_GET['action'] == 'edit') {
            ...
            validateJurusan($errors, $_POST, 'masukan-jurusan');
            validateDeskripsi($errors, $_POST, 'desk-jurusan');
            cekDuplikasi($errors, $DBH, 'jurusan', 'NAMA_JURUSAN', $_POST['masukan-jurusan'], 'jurusanss', '*Jurusan sudah ada');
            ...
            <table>
                ...
                ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($jurusan['DESKRIPSI_JURUSAN']); ?></td>
                        <td>
                            <div class="btn-aksi">
                                <a href="beranda_admin.php?action=edit&nama=<?= urlencode($jurusan['NAMA_JURUSAN']); ?>" class="edit">Edit</a>

                                <form action="proses_admin/proses_delete.php?action=delete&nama=<?= urlencode($jurusan['NAMA_JURUSAN']); ?>" method="post">
    ...
    <?php if ($tampilkan_edit_kebutuhan): ?>
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_GET['action']) && $_GET['action'] == 'edit_kebutuhan') {
            ...
            validateJurusan($errors, $_POST, 'nama-kebutuhan');
            validateDeskripsi($errors, $_POST, 'desk-kebutuhan');
            cekDuplikasi($errors, $DBH, 'siswa_kebutuhan_khusus', 'NAMA_KEBUTUHAN', $_POST['nama-kebutuhan'], 'kebutuhanss', '*Kebutuhan sudah ada');
            if ($errors) {
                $nama_lama_get = $_GET['nama'] ?? '';
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['jurusan-terpilih'])) {
        $jurusan_terpilih = $_POST['jurusan-terpilih'] ?? NULL;
        $id_siswa = $_POST['id_siswa'] ?? NULL;
        if ($jurusan_terpilih && $id_siswa) {
            jurusanTerpilih($DBH, $jurusan_terpilih, $id_siswa);
        ...
        <?php if ($tampilkan_tabel_siswa): ?>
            ...
            <table>
                ...
                <?php if (!empty($daftar_siswa)): ?>
                    ...
                    $no = 1;
                    $siswa_diproses = [];
                    require 'proses_admin/logika_jurusan.php';
                    foreach ($siswa_diproses as $siswa):
                    ?>
                        <tr>
                            <td><?= $no++; ?></td>
                            ...
                            <td>
                                <form action="beranda_admin.php" method="POST">
                                    ...
                                    <select name="jurusan-terpilih">

                                        <option value="">--BELUM DITETAPKAN--</option>
                                        <?php foreach ($siswa['ALL_JURUSAN'] as $jurusan_t): ?>
                                            <option value="<?= htmlspecialchars($jurusan_t) ?>" <?= ($siswa['JURUSAN_TERPILIH'] == $jurusan_t) ? 'selected' : ''; ?>>
                                                <?= htmlspecialchars($jurusan_t) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
