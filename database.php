...
function validateName(&$errors, $field_List, $field_name)
{
    ...
    if (empty(trim($field_List[$field_name] ?? ''))) {
        ...
        $errors[$field_name] = 'Masukkan nama dengan benar dan ';
    }
}
...
function validateUsername(&$errors, $field_List, $field_name)
{
    ...
    if (empty(trim($field_List[$field_name] ?? ''))) {
        ...
        $errors[$field_name] = 'username wajib kombinasi angka dan huruf(min.3)';
    }
}
...
function validateFoto(&$errors, $file_List, $field_name)
{
    ...
    if ($file['size'] > $maxSize) {
        $errors[$field_name] = '*Ukuran foto terlalu besar (Max 1MB)';
        return;
    }
...
function validateIjazah(&$errors, $file_List, $field_name)
{
    ...
    $maxSize = 1 * 1024 * 1024;
    if ($file['size'] > $maxSize) {
        $errors[$field_name] = '*Ukuran file terlalu besar (Maksimal 1MB)';
        return;
    }
...
{
    // Jika satu operasi gagal, semua dibatalkan
    $db->beginTransaction();

    try {
        // tabel anak
        $sql_child = "DELETE FROM manage_jurusan WHERE ID_JURUSAN = ?";

        $stmt_child = $db->prepare($sql_child);
        $stmt_child->execute([$id_jurusan]);

        // tabel indux
        $sql_parent = "DELETE FROM jurusan WHERE ID_JURUSAN = ?";

        $stmt_parent = $db->prepare($sql_parent);
        $stmt_parent->execute([$id_jurusan]);

        // jika semua berhasil, maka akan lanjut
        $db->commit();

        return $stmt_parent->rowCount();
...
function jurusanTerpilih($db, $jurusan_tetap, $id_siswa)
{
    $sql = "UPDATE profil_siswa SET JURUSAN_TERPILIH = ? WHERE ID_SISWA = ?";
    $stmt = $db->prepare($sql);
    $stmt->execute([
        $jurusan_tetap,
        $id_siswa
...
// Query jurusan admin last


// query kebutuhan admin first
...
// query daftar siswa first & pendaftaran
...
function getDaftarSiswa($db)
{
    ...
    FROM
        profil_siswa AS ps
    JOIN
        pendaftaran AS p ON ps.ID_PENDAFTARAN = p.ID_PENDAFTARAN
    JOIN
        wali_siswa AS ws ON ps.ID_WALI = ws.ID_WALI
    LEFT JOIN
        kebutuhan AS k ON ps.ID_SISWA = k.ID_SISWA
    LEFT JOIN
        siswa_kebutuhan_khusus AS ksk ON k.ID_SISWA_KEBUTUHAN = ksk.ID_SISWA_KEBUTUHAN
    LEFT JOIN
        manage_jurusan mj ON p.ID_PENDAFTARAN = mj.ID_PENDAFTARAN
    LEFT JOIN
        jurusan j ON mj.ID_JURUSAN = j.ID_JURUSAN
    ORDER BY
        ps.ID_SISWA";
